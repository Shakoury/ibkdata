...
export default function PayElectricityScreen() {
  ...
  const { data: providers, isLoading } = useQuery({
    ...
    queryFn: transactionService.getElectricityProviders,
  });

  const proceed = () => {
    if (!provider) { Alert.alert('Error', 'Select a provider'); return; }
    if (!meterNumber) { Alert.alert('Error', 'Enter meter number'); return; }
    if (!meterType) { Alert.alert('Error', 'Select meter type'); return; }
  ...
  const confirm = async () => {
    ...
    setLoading(true);
    try {
      await transactionService.payElectricity({
  ...
  if (step === 'success') {
    return (
      <View style={[styles.container, styles.center]}>
        ...
        <Text style={styles.successSub}>{formatNaira(amount)} electricity paid for meter {meterNumber}</Text>
    ...
    return (
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.pinContent}>
          ...
          <Text style={styles.subtitle}>Confirm {formatNaira(amount)} for meter {meterNumber}</Text>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        {isLoading ? <ActivityIndicator color={Colors.accent} /> : (
          <View style={styles.providerList}>
            {(providers ?? []).map((p: any) => (
              <TouchableOpacity key={p.id} style={[styles.providerBtn, provider?.id === p.id && styles.providerBtnActive]} onPress={() => setProvider(p)}>
        ...
        <View style={styles.meterRow}>
          {['PREPAID', 'POSTPAID'].map((t) => (
            <TouchableOpacity key={t} style={[styles.meterBtn, meterType === t && styles.meterBtnActive]} onPress={() => setMeterType(t)}>
              <Text style={[styles.meterText, meterType === t && styles.meterTextActive]}>{t}</Text>
            </TouchableOpacity>
          ))}
        </View>

        <Text style={styles.label}>Meter Number</Text>
        <TextInput style={styles.input} placeholder="Enter meter number" placeholderTextColor={Colors.muted} value={meterNumber} onChangeText={setMeterNumber} keyboardType="numeric" />

        <Text style={styles.label}>Amount</Text>
        <TextInput style={styles.input} placeholder="Min ₦100" placeholderTextColor={Colors.muted} value={amount} onChangeText={setAmount} keyboardType="numeric" />
...
const styles = StyleSheet.create({
  ...
  providerBtn: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, borderWidth: 1, borderColor: Colors.border, backgroundColor: Colors.white },
  providerBtnActive: { backgroundColor: Colors.accent, borderColor: Colors.accent },
  providerText: { fontSize: 13, fontWeight: '500', color: Colors.ink },
  providerTextActive: { color: Colors.white },
  meterRow: { flexDirection: 'row', gap: 12 },
  meterBtn: { flex: 1, paddingVertical: 12, borderRadius: 10, borderWidth: 1, borderColor: Colors.border, backgroundColor: Colors.white, alignItems: 'center' },
  meterBtnActive: { backgroundColor: Colors.accent, borderColor: Colors.accent },
  meterText: { fontSize: 13, fontWeight: '600', color: Colors.ink },
