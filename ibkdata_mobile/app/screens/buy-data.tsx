...
export default function BuyDataScreen() {
  ...
  const { data: plans, isLoading: plansLoading } = useQuery({
    ...
    queryFn: () => transactionService.getDataPlans(network),
    enabled: !!network,
  });
  ...
  const proceed = () => {
    ...
    if (!plan) { Alert.alert('Error', 'Select a data plan'); return; }
    if (plan.amount > balance) { Alert.alert('Insufficient Balance', 'Please fund your wallet first'); return; }
    setStep('pin');
  };
  ...
  const confirm = async () => {
    ...
    setLoading(true);
    try {
      await transactionService.buyData({ phone, network, plan_id: plan.id, pin });
      setStep('success');
    } catch (err) {
  ...
  if (step === 'success') {
    return (
      <View style={[styles.container, styles.center]}>
        ...
        <Text style={styles.successSub}>{plan?.name} sent to {phone}</Text>
    ...
    return (
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.pinContent}>
          ...
          <Text style={styles.subtitle}>Confirm {plan?.name} ({formatNaira(plan?.amount)}) to {phone}</Text>
          <TextInput
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <TextInput style={styles.input} placeholder="08012345678" placeholderTextColor={Colors.muted} value={phone} onChangeText={setPhone} keyboardType="phone-pad" maxLength={11} />

        <Text style={styles.label}>Network</Text>
        <View style={styles.networkRow}>
          {networks.map((n) => (
            <TouchableOpacity key={n} style={[styles.networkBtn, network === n && styles.networkBtnActive]} onPress={() => { setNetwork(n); setPlan(null); }}>
        ...
        {!network ? (
          <Text style={styles.hint}>Select a network to see plans</Text>
        ) : plansLoading ? (
          <ActivityIndicator color={Colors.accent} />
        ) : (
          <View style={styles.plansList}>
            {(plans ?? []).map((p: any) => (
              <TouchableOpacity key={p.id} style={[styles.planBtn, plan?.id === p.id && styles.planBtnActive]} onPress={() => setPlan(p)}>
                <Text style={[styles.planName, plan?.id === p.id && styles.planTextActive]}>{p.name} {p.data_volume}</Text>
                <Text style={[styles.planPrice, plan?.id === p.id && styles.planTextActive]}>{formatNaira(p.amount)}</Text>
              </TouchableOpacity>
            ))}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  plansList: { gap: 8 },
  planBtn: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: Colors.white, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: Colors.border },
  planBtnActive: { backgroundColor: Colors.accent, borderColor: Colors.accent },
  planName: { fontSize: 14, fontWeight: '500', color: Colors.ink },
